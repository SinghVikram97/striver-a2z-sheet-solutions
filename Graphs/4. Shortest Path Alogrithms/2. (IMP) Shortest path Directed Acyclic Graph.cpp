  ...
  public:
    vector<int> shortestPath(int V, int E, vector<vector<int>>& edges) {
        ...
        while(!mq.empty()){
            ...
            // update distance of adjacent vertex
            for(int i=0;i<adjList[front].size();i++){
                pair<int,int> neighbour=adjList[front][i];
                
                int neighbourVertex=neighbour.first;
                int neighbourDistanceFromFront = neighbour.second;
                
                int distanceFrontFromSrc = dist[front];
                
                if(distanceFrontFromSrc+neighbourDistanceFromFront<dist[neighbourVertex]){
                    dist[neighbourVertex]=distanceFrontFromSrc+neighbourDistanceFromFront;
                    // push this neighbour to queue again
                    // to update distances of its adjacent vertices 
                    // since distance from src to neighbour updated it would affect its adjacent vertices
                    mq.push(neighbourVertex);
                }
            }
        }
...
// Instead of pushing to the queue again
// We can make use of topological sort so that we find distances in order of depedencies